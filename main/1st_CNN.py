...
os.environ['CUDA_VISIBLE_DEVICES']='0'  # 只使用第二块GPU
...
#         temp = tf.reshape(tf.reshape(inputs[:, :, :, main_ch], none_ch_size) * main_mom + tf.reshape(neb, none_ch_size) * sub_mom, none_ch_size)
...
# 特征提取  （短时傅里叶变换）
result = tf.contrib.signal.stft(tf.cast(tf.reshape(xs, [-1, 21, 30*256]), tf.float32), frame_length=300, frame_step=58, fft_length=255)
x_image2 = tf.reshape(tf.transpose(tf.reshape(result, [batch_size, num_of_channels, -1]), [0, 2, 1]), [batch_size, 128, 128, num_of_channels])   # [num_of_samples, x, y, z]
...
mom_c1 = get_moments(fdc_c1, is_training=is_training, axes=[0, 1, 2, 3])
bn_c1 = bn_layer(fdc_c1, mom_c1[0], mom_c1[1], [128, 128, 21], [batch_size, 128, 128, 21])       # 128*128*21
...
mom_c2 = get_moments(fdc_c2, is_training=is_training, axes=[0, 1, 2, 3])
bn_c2 = bn_layer(fdc_c2, mom_c2[0], mom_c2[1], [64, 64, 21], [batch_size, 64, 64, 21])
...
mom_c3 = get_moments(fdc_c3, is_training=is_training, axes=[0, 1, 2, 3])
bn_c3 = bn_layer(fdc_c3, mom_c3[0], mom_c3[1], [32, 32, 21], [batch_size, 32, 32, 21])
...
mom_c4 = get_moments(fdc_c4, is_training=is_training, axes=[0, 1, 2, 3])
bn_c4 = bn_layer(fdc_c4, mom_c4[0], mom_c4[1], [16, 16, 21], [batch_size, 16, 16, 21])
...
with tf.name_scope('cross_entropy'):
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=ys, logits=prediction, name='cross_entropy'))
    # cross_entropy = tf.reduce_mean(-tf.reduce_sum(ys * tf.log(tf.clip_by_value(prediction, 1e-10,1.0)),axis=1))
...
train_writer = tf.summary.FileWriter(path, sess.graph)    # 输出tensorboard信息
...
for i in range(iteration):
    ...
    # 进行每步训练
    sess.run(train_step, feed_dict={xs: batch_xs, ys: batch_ys, it: i, lr: sess.run(learningRate), keep_prob: 0.7, is_training: True})
    ...
    if (i+1) % step_to_show_info == 0:
        ...
        print('Cross_entropy: ', sess.run(cross_entropy, feed_dict={xs: batch_xs, ys: batch_ys, keep_prob: 1, is_training: False}))
        ...
        saver.save(sess, save_path+'1st_CNN-model', global_step=i+1)  
        num_of_TN = 0
        num_of_FP = 0
        num_of_FN = 0
        num_of_TP = 0
    ...
    if (i+1) % step_to_record_info == 0:
        train_result = sess.run(merged, feed_dict={xs: batch_xs, ys: batch_ys, it: i, lr: sess.run(learningRate), keep_prob: 1, is_training: False})
