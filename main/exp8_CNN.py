...
xs = tf.compat.v1.placeholder(tf.float32, [None, input_size], name='xs')
ys = tf.compat.v1.placeholder(tf.float32, [None, output_size], name='ys')
...
# 假如需要保存y，以便在预测时使用
tf.compat.v1.add_to_collection('pred_network', prediction)
...
for i in range(iteration):
    ...
    if i % 100 == 0:
        ...
        saver.save(sess, 'save/exp8_CNN/exp8_CNN-model', global_step=i)      
